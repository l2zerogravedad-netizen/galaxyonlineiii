...
/**
 * Intenta interceptar misiles con PPCs.
 * Cada PPC tiene 55% chance de destruir UN misil.
 *
 * @param ppcCount - Numero de PPCs
 * @param incomingMissiles - Numero de misiles entrantes
 * @param rng - Generador RNG
 * @returns Numero de misiles destruidos
 */
export function attemptPPCIntercept(
  ...
  let destroyed = 0;
  const effectivePPCs = Math.min(ppcCount, incomingMissiles);

  for (let i = 0; i < effectivePPCs; i++) {
    const roll = rng ? rng.chance(0.55) : Math.random() < 0.55;
    if (roll) {
      destroyed++;
    }
  }
