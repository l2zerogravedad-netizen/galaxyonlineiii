...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const fleet = await prisma.fleet.findFirst({
      where: { id: params.id, empireId: user.empireId },
      include: {
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      const formations = await tx.fleetFormation.findMany({ where: { fleetId: params.id } });
      for (const f of formations) {
        await tx.ship.update({ where: { id: f.shipId }, data: { quantity: { increment: f.quantity }, fleetId: null } });
      }
      await tx.fleetFormation.deleteMany({ where: { fleetId: params.id } });
      await tx.fleet.delete({ where: { id: params.id } });
    });
